...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="h4 fw-bold mb-1">Edit Data Penduduk</h2>
        <p class="text-muted mb-0">Perbarui data kependudukan <?= esc($penduduk['nama_lengkap']) ?>.</p>
    </div>
...
<div class="card bg-white p-4 shadow-sm">
    <form action="<?= base_url($baseRoute . '/edit/' . $penduduk['id']) ?>" method="post">
        <?= csrf_field() ?>
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-6">
                ...
                <input type="text" name="nik" maxlength="16" class="form-control <?= ($validation && $validation->hasError('nik')) ? 'is-invalid' : '' ?>" placeholder="16 Digit NIK KTP" value="<?= old('nik', $penduduk['nik']) ?>" required>
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="no_kk" maxlength="16" class="form-control <?= ($validation && $validation->hasError('no_kk')) ? 'is-invalid' : '' ?>" placeholder="16 Digit Nomor KK" value="<?= old('no_kk', $penduduk['no_kk']) ?>" required>
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="nama_lengkap" class="form-control <?= ($validation && $validation->hasError('nama_lengkap')) ? 'is-invalid' : '' ?>" placeholder="Nama lengkap tanpa singkatan" value="<?= old('nama_lengkap', $penduduk['nama_lengkap']) ?>" required>
            ...
            <div class="col-md-3">
                ...
                <input type="text" name="tempat_lahir" class="form-control <?= ($validation && $validation->hasError('tempat_lahir')) ? 'is-invalid' : '' ?>" placeholder="Kota/Kab Tempat Lahir" value="<?= old('tempat_lahir', $penduduk['tempat_lahir']) ?>" required>
            ...
            <div class="col-md-3">
                ...
                <input type="date" name="tanggal_lahir" class="form-control <?= ($validation && $validation->hasError('tanggal_lahir')) ? 'is-invalid' : '' ?>" value="<?= old('tanggal_lahir', $penduduk['tanggal_lahir']) ?>" required>
            ...
            <div class="col-md-3">
                ...
                <select name="golongan_darah" class="form-select">
                    ...
                    <?php foreach(['A', 'B', 'AB', 'O'] as $goldar): ?>
                        <option value="<?= $goldar ?>" <?= old('golongan_darah', $penduduk['golongan_darah']) === $goldar ? 'selected' : '' ?>><?= $goldar ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-md-3">
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-4">
                ...
                <select name="status_hubungan_kk" class="form-select">
                    ...
                    <?php foreach(['Kepala Keluarga', 'Istri', 'Anak', 'Menantu', 'Cucu', 'Orang Tua', 'Mertua', 'Famili Lain', 'Pembantu', 'Lainnya'] as $hub): ?>
                        <option value="<?= $hub ?>" <?= old('status_hubungan_kk', $penduduk['status_hubungan_kk']) === $hub ? 'selected' : '' ?>><?= $hub ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-md-4">
                ...
                <select name="pendidikan" class="form-select">
                    ...
                    <?php foreach(['Tidak Sekolah', 'SD', 'SMP', 'SMA/SMK', 'D1', 'D2', 'D3', 'D4', 'S1', 'S2', 'S3'] as $pddk): ?>
                        <option value="<?= $pddk ?>" <?= old('pendidikan', $penduduk['pendidikan']) === $pddk ? 'selected' : '' ?>><?= $pddk ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-md-4">
                ...
                <select name="pekerjaan" id="pekerjaan" class="form-select" placeholder="Cari atau pilih pekerjaan...">
                    ...
                    <?php 
                        $currentJob = old('pekerjaan', $penduduk['pekerjaan'] ?? '');
                        $hasMatched = false;
                    ?>
                    <?php if (!empty($listPekerjaan)): ?>
                        <?php foreach ($listPekerjaan as $pek): ?>
                            <?php 
                                $isSelected = ($currentJob === $pek['nama']);
                                if ($isSelected) $hasMatched = true;
                            ?>
                            <option value="<?= esc($pek['nama']) ?>" <?= $isSelected ? 'selected' : '' ?>>
                                <?= esc($pek['nama']) ?>
                            </option>
                        <?php endforeach; ?>
                    <?php endif; ?>
                    <?php if (!$hasMatched && !empty($currentJob)): ?>
                        <option value="<?= esc($currentJob) ?>" selected><?= esc($currentJob) ?></option>
                    <?php endif; ?>
                </select>
            ...
            <div class="col-md-6">
                ...
                <select name="kewarganegaraan" class="form-select">
                    ...
                    <option value="WNA" <?= old('kewarganegaraan', $penduduk['kewarganegaraan'] ?? 'WNI') === 'WNA' ? 'selected' : '' ?>>WNA (Warga Negara Asing)</option>
                </select>
            </div>
            <div class="col-md-6">
                ...
                <select name="status_penduduk" class="form-select" required>
                    <option value="Tetap" <?= old('status_penduduk', $penduduk['status_penduduk']) === 'Tetap' ? 'selected' : '' ?>>Tetap (Warga Asli)</option>
                    <option value="Sementara" <?= old('status_penduduk', $penduduk['status_penduduk']) === 'Sementara' ? 'selected' : '' ?>>Sementara (Pendatang / Kos)</option>
                    <option value="Pindah" <?= old('status_penduduk', $penduduk['status_penduduk']) === 'Pindah' ? 'selected' : '' ?>>Pindah Keluar</option>
                    <option value="Meninggal" <?= old('status_penduduk', $penduduk['status_penduduk']) === 'Meninggal' ? 'selected' : '' ?>>Meninggal Dunia</option>
                </select>
            </div>
        </div>
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-4">
                ...
                <input type="text" name="dusun" class="form-control" placeholder="Nama Dusun / Kampung" value="<?= old('dusun', $penduduk['dusun']) ?>">
            </div>
            <div class="col-md-4">
                <label class="form-label fw-semibold">RT</label>
                <input type="text" name="rt" maxlength="5" class="form-control" placeholder="001" value="<?= old('rt', $penduduk['rt']) ?>">
            </div>
            <div class="col-md-4">
                <label class="form-label fw-semibold">RW</label>
                <input type="text" name="rw" maxlength="5" class="form-control" placeholder="001" value="<?= old('rw', $penduduk['rw']) ?>">
            </div>
            <div class="col-12">
                <label class="form-label fw-semibold">Alamat Lengkap (Jalan, No Rumah, Patokan)</label>
                <textarea name="alamat_lengkap" class="form-control" rows="2" placeholder="Jl. Mawar No. 10..."><?= old('alamat_lengkap', $penduduk['alamat_lengkap']) ?></textarea>
            </div>
        </div>

        <div class="d-flex justify-content-end gap-2 pt-3 border-top">
            <a href="<?= base_url($baseRoute) ?>" class="btn btn-light px-4">Batal</a>
            <button type="submit" class="btn btn-primary px-4"><i class="bi bi-check-lg me-1"></i> Perbarui Data Penduduk</button>
        </div>
    </form>
</div>
<?= $this->endSection() ?>
