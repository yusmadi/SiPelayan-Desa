...
<div class="mb-4">
    ...
    <h2 class="h3 font-weight-bold mb-1">Edit Laporan Transparansi <?= $istilah ?></h2>
    <p class="text-muted mb-0">Perbarui rincian data pendapatan, belanja 5 bidang, atau berkas lampiran resmi <?= strtolower($sebutan) ?>.</p>
</div>
...
<form action="<?= base_url('operator/transparansi/update/' . $anggaran['id']) ?>" method="post" enctype="multipart/form-data">
    <?= csrf_field() ?>
                ...
                <div class="card-body p-4">
                    <div class="row g-3 mb-3">
                        <div class="col-md-4">
                            ...
                            <input type="number" class="form-control" id="tahun_anggaran" name="tahun_anggaran" value="<?= old('tahun_anggaran', $anggaran['tahun_anggaran']) ?>" required min="2020" max="2030">
                        </div>
                        <div class="col-md-4">
                            ...
                            <select class="form-select" id="periode_anggaran" name="periode_anggaran" required>
                                <?php foreach ($periodeList as $val => $label): ?>
                                    <option value="<?= $val ?>" <?= (old('periode_anggaran', $anggaran['periode_anggaran']) === $val) ? 'selected' : '' ?>><?= $label ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-4">
                            <label for="nomor_pengumuman" class="form-label fw-semibold">No. Perdes / SK Pengesahan</label>
                            <input type="text" class="form-control font-monospace" id="nomor_pengumuman" name="nomor_pengumuman" value="<?= old('nomor_pengumuman', $anggaran['nomor_pengumuman'] ?? '') ?>" placeholder="Contoh: PERDES-2026/01">
                        </div>
                    </div>

                    <div class="mb-3">
                        <label for="judul" class="form-label fw-semibold">Judul Laporan Transparansi <span class="text-danger">*</span></label>
                        <input type="text" class="form-control form-control-lg fs-6" id="judul" name="judul" value="<?= old('judul', $anggaran['judul']) ?>" required>
                    </div>

                    <div class="mb-3">
                        <label for="ringkasan" class="form-label fw-semibold">Ringkasan Eksekutif</label>
                        <textarea class="form-control" id="ringkasan" name="ringkasan" rows="2"><?= old('ringkasan', $anggaran['ringkasan']) ?></textarea>
                    </div>

                    <div class="mb-0">
                        <label for="konten" class="form-label fw-semibold">Narasi & Penjelasan Laporan</label>
                        <textarea class="form-control" id="konten" name="konten" rows="4"><?= old('konten', $anggaran['konten']) ?></textarea>
                    </div>
                </div>
            </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" class="form-control" id="total_pendapatan" name="total_pendapatan" value="<?= old('total_pendapatan', (float) $anggaran['total_pendapatan']) ?>" required min="0">
                            </div>
                        </div>
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <span class="input-group-text bg-light">Rp</span>
                                <input type="number" class="form-control" id="realisasi_pendapatan" name="realisasi_pendapatan" value="<?= old('realisasi_pendapatan', (float) $anggaran['realisasi_pendapatan']) ?>" min="0">
                            </div>
                        </div>
                        <div class="col-12">
                            <label for="pendapatan_ket" class="form-label fw-semibold">Keterangan Sumber Pendapatan</label>
                            <input type="text" class="form-control" id="pendapatan_ket" name="pendapatan_ket" value="<?= old('pendapatan_ket', $rPendapatan['keterangan'] ?? '') ?>">
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <div class="row g-2">
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group input-group-sm">
                                        ...
                                        <input type="number" class="form-control input-belanja-anggaran" name="belanja_<?= $key ?>_anggaran" value="<?= old("belanja_{$key}_anggaran", (float) ($bData['anggaran'] ?? 0)) ?>" min="0" oninput="hitungTotalBelanja()">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group input-group-sm">
                                        <span class="input-group-text">Rp</span>
                                        <input type="number" class="form-control input-belanja-realisasi" name="belanja_<?= $key ?>_realisasi" value="<?= old("belanja_{$key}_realisasi", (float) ($bData['realisasi'] ?? 0)) ?>" min="0" oninput="hitungTotalBelanja()">
                                    </div>
                                </div>
                                <div class="col-12 mt-2">
                                    <input type="text" class="form-control form-control-sm" name="belanja_<?= $key ?>_ket" value="<?= old("belanja_{$key}_ket", $bData['keterangan'] ?? '') ?>" placeholder="Uraian singkat kegiatan...">
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
                ...
                <div class="card-body p-4">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <span class="input-group-text bg-light">Rp</span>
                                <input type="number" class="form-control" id="total_pembiayaan" name="total_pembiayaan" value="<?= old('total_pembiayaan', (float) $anggaran['total_pembiayaan']) ?>" min="0">
                            </div>
                        </div>
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <span class="input-group-text bg-light">Rp</span>
                                <input type="number" class="form-control" id="realisasi_pembiayaan" name="realisasi_pembiayaan" value="<?= old('realisasi_pembiayaan', (float) ($rPembiayaan['realisasi'] ?? $anggaran['total_pembiayaan'])) ?>" min="0">
                            </div>
                        </div>
                        <div class="col-12">
                            <label for="pembiayaan_ket" class="form-label fw-semibold">Keterangan Pembiayaan</label>
                            <input type="text" class="form-control" id="pembiayaan_ket" name="pembiayaan_ket" value="<?= old('pembiayaan_ket', $rPembiayaan['keterangan'] ?? '') ?>">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm mb-4 sticky-top" style="top: 80px; z-index: 10;">
                ...
                <div class="card-body p-3">
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <span class="fw-bold text-dark" id="dispTotalBelanja">Rp 0</span>
                    </div>
                    <div class="d-flex justify-content-between mb-2">
                        <span class="text-muted small">Total Realisasi Belanja:</span>
                        <span class="fw-bold text-success" id="dispTotalRealisasi">Rp 0</span>
                    </div>
                    <div class="mb-3">
                        <div class="d-flex justify-content-between mb-1">
                            <span class="text-muted small">Tingkat Capaian:</span>
                            <span class="fw-bold text-primary" id="dispPersen">0%</span>
                        </div>
                        <div class="progress" style="height: 10px;">
                            <div class="progress-bar bg-success progress-bar-striped progress-bar-animated" id="dispProgressBar" role="progressbar" style="width: 0%"></div>
                        </div>
                    </div>
                    ...
                    <!-- Berkas Lampiran PDF -->
                    <div class="mb-3">
                        <label class="form-label fw-semibold small">Lampiran Dokumen Resmi</label>
                        <?php if (!empty($anggaran['lampiran_path'])): ?>
                            <div class="alert alert-white bg-white border d-flex align-items-center justify-content-between p-2 mb-2">
                                <div class="d-flex align-items-center text-truncate me-2">
                                    ...
                                    <div>
                                        <span class="fw-semibold text-dark small d-block"><?= esc($anggaran['lampiran_nama'] ?? 'Berkas PDF') ?></span>
                                        <a href="<?= base_url($anggaran['lampiran_path']) ?>" target="_blank" class="small text-primary text-decoration-none" style="font-size: 0.72rem;">
                                            Lihat Berkas Aktif
                                        </a>
                                    </div>
                                </div>
                                <div class="form-check">
                                    ...
                                    <label class="form-check-label text-danger small" for="hapus_lampiran" style="font-size: 0.72rem;">
                                        Hapus
                                    </label>
                                </div>
                            </div>
                        ...
                        <small class="text-muted" style="font-size: 0.72rem;">Unggah file baru untuk mengganti lampiran saat ini.</small>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="is_published" id="status1" value="1" <?= old('is_published', (string) $anggaran['is_published']) === '1' ? 'checked' : '' ?>>
                            <label class="form-check-label text-success small fw-semibold" for="status1">
                                <i class="bi bi-broadcast me-1"></i> Terbitkan Sekarang
                            </label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="is_published" id="status0" value="0" <?= old('is_published', (string) $anggaran['is_published']) === '0' ? 'checked' : '' ?>>
